...
These are three independent concepts. The rest of this page follows from keeping them apart.
...
- **Private by default.** `push`, `package`, and `package-thin` all create a **new** repo as
  **private**. A bundle can point at proprietary weights, so a repo is not made public by
  ...
  is always announced (`! Changed visibility …`).
- **`--publish` implies `--public`.** The catalog is a public index that can only see public
  repos, so publishing makes the repo public and lists it in one step. You do not pass both
  flags. `--public` on its own makes the repo public **without** listing it, so it can be shared
  by link without being advertised. `--publish` combined with `--private` is a contradiction and
  is refused. Publishing is a pointer opt-in: the catalog stores none of your content and the
...
The catalog, including its web front end and its indexer, lives in a dedicated repo,
...
reports a verdict on `pull`, `serve`, and `info` (`compare()` in
[`compat.py`](../src/tt_kernel/compat.py)). An `arch` mismatch is fatal, because the binaries
target a different instruction set architecture (ISA). Other mismatches (for example too few
chips for the chosen profile) are non-fatal and overridable with `--force`. Because each bundle
builds its own venv, there is no host `ttnn` or vLLM version to gate against. v5.1 container
