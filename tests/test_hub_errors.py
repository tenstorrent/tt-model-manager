...
from tt_model.hub import classify_hub_error
...
def test_404_names_both_possibilities():
    ...
    assert d["cause"] == "no such package, or no access"
...
def test_missing_manifest_is_not_a_bundle():
    d = classify_hub_error(
        _exc("EntryNotFoundError", "tt-model.yaml does not exist"), "x/y")
    assert d["cause"] == "not a tt-model package"
...
def test_every_branch_returns_the_full_card_contract():
    ...
    cases = [
        ...
        _exc("EntryNotFoundError", "tt-model.yaml missing"),
...
def test_cli_renders_a_card_not_a_traceback(monkeypatch):
    ...
    from tt_model import cli, hub

    def boom(*a, **k):
        raise _exc("RepositoryNotFoundError", REPO_NOT_FOUND, 404)

    monkeypatch.setattr(hub, "download_package", boom)
    res = runner.invoke(cli.app, ["pull", "x/y"])
    assert res.exit_code == 1
    assert "no such package, or no access" in res.output
